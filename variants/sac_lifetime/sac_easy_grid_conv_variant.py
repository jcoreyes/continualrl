...
def gen_network(variant, action_dim, layer_size, policy=False):
    final_network_kwargs = dict(
        # +1 for health
        input_size=variant['img_conv_kwargs']['output_size'] + variant['full_img_conv_kwargs'][
            'output_size'] + 1,
    ...
    return FoodNetworkEasy(
        ...
        sizes=[
            ...
            variant['full_img_conv_kwargs']['input_width'] * variant['full_img_conv_kwargs'][
                'input_height'] * variant['full_img_conv_kwargs']['input_channels'],
            # health dim
            1
        ]
    )
