...
class WorldObj:
    ...
    def step(self):
        self.age += 1
        # Return False if time to die. By default, lifespan is 0 so age will never be lifespan,
        # and the obj is permanent
...
class Monster(WorldObj):
    ...
    def get_action(self, goal_pos):
        if random.random() < self.eps:
            ...
            _, dir = min(DIR_TO_VEC.items(),
                         key=lambda item: np.linalg.norm(item[1] + self.cur_pos - goal_pos))
        return dir
...
class MiniGridAbsoluteEnv(gym.Env):
    ...
    def _reward(self):
        ...
        """
        return 1 - 0.9 * (
        self.step_count / self.max_steps)  # if self.done_cond(self.agent_pos) else 0
    ...
    def place_obj(self, obj, top=None, size=None, reject_fn=None):
        ...
        while True:
            num_tries += 1
            # 5 is rough approximation to make sure that we really are out of space if we don't
            # find an empty spot
