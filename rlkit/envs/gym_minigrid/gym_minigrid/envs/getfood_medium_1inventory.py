...
class FoodEnvMedium1Inv(FoodEnvBase):
    ...
    def step(self, action):
        ...
        extra_obs_count_string = np.concatenate(
            (pantry_obs.sum(axis=0), shelf_obs.sum(axis=0))).tostring()
        ...
        # Exploration bonuses
        if self.cbe:
            self.obs_count[extra_obs_count_string] = self.obs_count.get(extra_obs_count_string,
                                                                        0) + 1
    ...
    def get_make_reward(self):
        ...
        if self.make_rtype == 'sparse':
            ...
            carry_idx = self.make_sequence.index(
                self.carrying.type) if self.carrying and self.carrying.type in self.make_sequence\
				else -1
            place_idx = self.make_sequence.index(
                self.last_placed_on.type) if self.last_placed_on and self.last_placed_on.type in \
																	 self.make_sequence else -1
...
class FoodEnvMedium1InvCap1000InitDecayFullObsLifespan200Task(FoodEnvMedium1Inv):
    def __init__(self):
        super().__init__(health_cap=1000, food_rate_decay=0.01, lifespan=200, fully_observed=True,
                         task='make axe',
                         make_rtype='dense',
                         init_resources={
                             'axe': 8,
...
class FoodEnvMedium1Inv1TierDenseReward16(FoodEnvMedium1Inv):
    def __init__(self):
        super().__init__(grid_size=16, health_cap=1000, gen_resources=False, fully_observed=True,
                         task='make energy',
                         make_rtype='dense',
                         init_resources={
                             'energy': 4
...
class FoodEnvMedium1Inv3TierDenseReward(FoodEnvMedium1Inv):
    def __init__(self):
        super().__init__(health_cap=1000, lifespan=200, fully_observed=True, task='make wood',
                         woodfood=False,
                         make_rtype='dense',
                         init_resources={
                             'food': 15,
