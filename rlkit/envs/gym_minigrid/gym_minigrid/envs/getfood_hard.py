...
class FoodEnvHard(FoodEnvBase):
    ...
    def place_items(self):
        ...
        if np.random.binomial(1, 1 / (8 * self.food_rate)):
            ...
            monster = Monster(
                np.array(self._rand_pos(1, self.grid_size - 2, 1, self.grid_size - 2)),
                self,
                12 * self.food_rate,
                0.5)
    ...
    def dead(self):
        return super().dead() or any(
            [np.allclose(monster.cur_pos, self.agent_pos) for monster in self.monsters])
