...
class SACDiscreteTrainer(TorchTrainer):
    ...
    def train_from_torch(self, batch):
        ...
        if ptu.gpu_enabled():
            log_pis = torch.stack([dist.log_prob(torch.tensor(ac).cuda()) for ac in range(self.policy.action_dim)]).permute(1, 0)
        else:
            log_pis = torch.stack([dist.log_prob(torch.tensor(ac)) for ac in range(self.policy.action_dim)]).permute(1, 0)
        ...
        if self._need_to_update_eval_statistics:
            ...
            # TODO suvansh commented out since we no longer have a mean and std
            # self.eval_statistics.update(create_stats_ordered_dict(
            #     'Policy mu',
